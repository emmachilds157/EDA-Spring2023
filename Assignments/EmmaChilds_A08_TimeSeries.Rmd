...
GaringerOzone_2010s <- GaringerOzone_new %>%
...
```{r}
#9
GaringerOzone.monthly <- GaringerOzone_2010s %>%
  mutate(Month = month(Date), Year = year(Date)) %>%
  mutate(Month_Year = my(paste0(Month,"-",Year))) %>%
  group_by(Year, Month) %>%
  summarise(mean_monthly_ozone = mean(Daily.Max.8.hour.Ozone.Concentration))
...
```{r}
#10
GaringerOzone.daily.ts <- ts(GaringerOzone_2010s$Ozone_clean, start = c(2010,1), frequency = 365)
GaringerOzone.monthly.ts <- ts(GaringerOzone.monthly$mean_monthly_ozone, start = c(2010,1), frequency = 12)
...
#I can't get this to run for some reason and no amount of troubleshooting seems like it is going to fix it. I'm going to comment out the next line or two of code because I know the code I need, but I can't run it with this plot not working.
...
#See description above. I'm so appreciative of the extension Dr. Launa provided, but I'm still floundering on this assignment and will follow up with the TAs to go over it more fully. 
