...
#Imported EPA data (from the Processed_KEY folder) & fixed dates
epa_data <- read.csv(
...
#Computed mean PM2.5 concentration for each county, sorted by value
...
#Computed mean ozone concentration for each county, sorted by value
...
#Used the `kable` function to produce the 2 tables below
#Table 1 for mean PM2.5 concentration by county
kable(summary_PM25,caption = "Mean Particulates (2.5mm)",
      col.names=c("$\\mu g/m^3$", "County"))

#Table 2 for mean ozone concentration by county
kable(summary_PM25,caption = "Mean Ozone",
      col.names=c("$\\mu g/m^3$", "County"))
...
ggplot(Ozone2018, aes(x=factor(Month, level=1:12, labels=month.abb),y=Ozone)) +
  geom_boxplot() +
  labs(
    title ='Ozone Distribution by Month (2018)',
    x = 'Month',
    y = 'Ozone (mu g/m^3)'
  )

ggplot(Ozone2019, aes(x=factor(Month, level=1:12, labels=month.abb),y=Ozone)) +
  geom_boxplot() +
  labs(
    title ='Ozone Distribution by Month (2019)',
    x = 'Month',
    y = 'Ozone (mu g/m^3)'
  )

#Built two ggplots from these subsetted datasets
```
