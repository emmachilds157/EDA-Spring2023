...
#Experimenting....will explain more - EMMA tell come back to this 
...
#come back and trouble shoot if this lm() was correct, check with classmates
...
#15
TukeyHSD(NTL.anova)
Tuk.lakemeans <- TukeyHSD(NTL.anova)
Tuk.table <-
Tuk.lakemeans$lakename %>%
  as.data.frame() %>%
  arrange(abs(diff))
row.names(Tuk.table)
Tuk.table %>%
  filter(str_detect(row.names(Tuk.table),"Peter Lake"))
