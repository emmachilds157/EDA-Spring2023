...
#noticing no values for Jan, March, April or December

#Plot how many Lake observations occur each month
ggplot(PeterPaul.chem.nutrients, aes(x=month)) + 
  geom_bar()
#need months to be factors so that we have more control over the x-axis
...
ggplot(PeterPaul.chem.nutrients, aes(x=factor(month,level=1:12,labels = month.abb))) + 
  ...
  labs(
    ...
    x="month")
#have to figure out how to override automatic settings so that we can display all 12 levels in the bar plot. x is the axis, so we have to do scale_x_discrete, then drop=FALSE keeps it from dropping the levels that aren't represented. This then depicts all 12 months - a visualization that communicates much more effectively than before. 
