...
#the only thing we have to change is the site number and period number in this above URL. The code below does not need to change because it will all refer back to this first line where we choose the data we pulled.
...
#you can sign up for a key at this website...

```{r}
#Apply the API key
#census_api_key("d4ad7874c996c7a83fec0a81466309ddc07d8e22a", install = TRUE)
#this is John's key above, we can uncomment and play around with it.
...
nc_pop <- 
  get_acs(geography = "county",
          ...
          state = "NC",
          geometry = TRUE)
#this creates a spatial dataframe from population data and then we can create a quick map (see below)
...
the_states <- html_nodes(the_website,"COPY the tag from the gadget selector tool")
  #looking to create a vector of all the state names, need to come back and finish so I get a list of 52 (have to load the gadget selector tool)

#the_states <- html_nodes(the_website,'td:nth_child(1)') %>% html_text()
the_price <- 
the_capacity <- 
net_generation <- 
total_retail <- 
