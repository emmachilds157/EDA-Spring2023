...
#summary(Rainfall_BR)
#sum(is.na(Rainfall_BR)) - this is problematic bc if you have an absent row, the is.na will not catch it.
#summary(Rainfall_BR_processed)
#these lines of code above are all the ones I tried in class, but the one below not commented is what Luana suggested we use
...
#ggplot(BR_complete, aes(x=))
#geom_line() +
 # labs(x=)
#method=lm? - finish code!
...
#more general - allows us to find the first date by year or month in a dataset if they aren't organized
f_year <- year(first(BR_complete$Date))
f_month <- month(first(BR_complete$Date))
#you could then put f_year or f_month into the ts code above if you didn't know the exact year or month when you first type the code - allows you to automate the process 
...
#ggplot(Rainfall_BR_components) +
 ...
 # ylab(expression("Rainfall + FINISH UNITS)"))
