...
O3.density<- 
  ...
  labs(fill="", 'Ozone AQI values,') #FINISH
print(O3.density) 
...
O3.boxplot <-
  ...
  geom_boxplot()+
  stat_summary(geom="text", fun=max, vjust=-1, size=4, labels=c("a", "b"))
print(O3.boxplot)
#FINISH
...
PM2.5regression <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(PM2.5regression)
...
EPAirCorr <- cor(EPAair.subset)
corrplot(EPAirCorr, method = "ellipse")
corrplot.mixed(EPAirCorr, upper = "ellipse")
