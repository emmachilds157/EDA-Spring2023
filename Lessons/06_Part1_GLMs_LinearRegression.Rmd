...
> There is a strong correlation between two variables. There is a strong negative correlation between irradiance and depth. Lower lights at greater depths. See correlation test for specific outputs. 
...
#ylim is not changing the plotting window, but it shifts the display window so we aren't distracted by the outlier. 
```
...
plot(TPregression)
par(mfrow = c(1,1))
#these are the residual plots - don't forget, Emma.
```
...
#this is a symmetric matrix, so you only need to print the upper diagonal 
