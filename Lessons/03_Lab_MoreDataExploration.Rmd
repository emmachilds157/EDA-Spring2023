...
e <- c("red", "white", "red", NA) # What type of vector? text
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector? Boulean 
...
```{r DataExercise-1a}
vectorhigh <- c(64,71,72,63,51,52,61,64,67,73)
vectorhigh
vectorlow <-c(42,49,60,46,32,32,41,47,52,54)
vectorlow
```
...
```{r DataExercise-1b}
temp.conversation.function <- function(F){
  C <- round(((F-32)*(5/9)),2)
return(C)
}
highC <- temp.conversation.function(vectorhigh)
highC
lowC <- temp.conversation.function(vectorlow)
lowC
```
...
```{r DataExercise-1c}
data.frame <- cbind(vectorhigh, vectorlow, highC, lowC)
colnames(data.frame) <- c("highF","lowF", "highC","lowC")
```
