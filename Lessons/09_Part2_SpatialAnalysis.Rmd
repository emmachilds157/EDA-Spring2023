...
#the mapviewOptions (fgb = FALSE) is needed for Macs, otherwise mapview acts weird
...
huc8__subbasin_sf <- st_read('./Data/Spatial/NCHUC8.shp') %>%
...
ggplot() +
  ...
  geom_sf(data = huc8__subbasin_sf, fill = "purple")
#needs the "data = ..." for some reason, which you don't need in mapView

mapview(huc8_sf, col.regions = "orange") +
  mapview(huc8__subbasin_sf, col.regions = "purple", alpha = 1)
...
#substr is substring, which takes the first 8 characters of a column and puts it in a new column
...
#Selecting counties that intersect the Neuse

neuse_intersect_3 <- counties_sf_utm %>%
  st_filter(neuse_sf)
#eliminating the predicate???
